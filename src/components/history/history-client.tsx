...
export function HistoryClient({
  ...
  return <>
    ...
    <div className="mt-6 flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-white p-3"><label className="text-sm font-semibold text-slate-600">เลือกวันที่ <input type="date" value={dateFilter} onChange={(event) => setDateFilter(event.target.value)} className="ml-2 h-10 rounded-lg border border-slate-200 px-3" /></label>{dateFilter && <button type="button" onClick={() => setDateFilter("")} className="h-10 rounded-lg bg-slate-100 px-3 text-sm font-semibold text-slate-600">แสดงทั้งหมด</button>}</div>
    <section className="mt-3 max-h-[65dvh] overflow-y-auto rounded-xl border border-slate-200 bg-white">
      {visibleItems.length ? groups.map(({ key, label, items: dateItems }) => {
        ...
        return <div key={key} className="border-b border-slate-200 last:border-b-0">
          ...
          {!collapsed && <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm"><thead className="border-y border-slate-200 bg-white text-left text-xs text-slate-500"><tr><th className="px-4 py-2 font-medium">เลขที่เอกสาร</th><th className="px-4 py-2 font-medium">{statusColumnLabel}</th><th className="px-4 py-2 font-medium">ยุทโธปกรณ์</th><th className="px-4 py-2 font-medium">จำนวน</th><th className="px-4 py-2 font-medium">ต้นทาง → ปลายทาง</th><th className="px-4 py-2 font-medium">เวลา</th><th className="px-4 py-2 text-right font-medium">รายละเอียด</th></tr></thead><tbody>{dateItems.map((item) => {
            const returned = item.movementType === "return";
            const defective = item.movementType === "defect";
            const overdue = item.movementType === "borrow" && item.status.toLowerCase() === "overdue";
            const openReceipt = () => { setDownloads(false); setSelected(item); };
            return <tr key={`${item.movementType}:${item.id}`} tabIndex={0} onClick={openReceipt} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openReceipt(); } }} className="cursor-pointer border-t border-slate-100 transition hover:bg-blue-50 focus-visible:bg-blue-50 focus-visible:outline-none"><td className="max-w-[150px] px-4 py-2.5"><span className="block overflow-x-auto whitespace-nowrap font-mono text-xs text-slate-500">{item.id}</span></td><td className="px-4 py-2.5"><span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-xs font-semibold ${defective ? maintenanceStatusClass(item.status) : returned ? "bg-emerald-50 text-emerald-700" : overdue ? "bg-red-50 text-red-700" : "bg-amber-50 text-amber-700"}`}>{defective ? <Wrench className="size-3.5" /> : returned ? <PackageCheck className="size-3.5" /> : <PackagePlus className="size-3.5" />}{defective ? historyStatus(item.status) : returned ? "คืน" : overdue ? "เกินกำหนด" : "เบิก"}</span></td><td className="max-w-[240px] truncate px-4 py-2.5 font-semibold">{item.equipmentName}</td><td className="px-4 py-2.5">{item.quantity.toLocaleString("th-TH")}</td><td className="max-w-[260px] truncate px-4 py-2.5 text-slate-600">{defective ? item.ownerCompanyName || "-" : `${item.ownerCompanyName} → ${item.borrowerCompanyName}`}</td><td className="whitespace-nowrap px-4 py-2.5 text-slate-500">{formatTime(item.date)}</td><td className="px-4 py-2 text-right"><button type="button" onClick={(event) => { event.stopPropagation(); openReceipt(); }} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 text-xs font-semibold transition hover:border-blue-400 hover:text-blue-600"><Eye className="size-3.5" />ดูใบเสร็จ</button></td></tr>;
          })}</tbody></table></div>}
        </div>;
        ...
        <div className="popup-panel max-h-[95vh] w-full max-w-md overflow-y-auto rounded-t-[30px] bg-slate-100 p-4 sm:rounded-[30px]">
          <div ref={receiptRef}>
            <ReceiptDocument
              ...
              status={selected.movementType === "borrow" ? "บันทึกการเบิกแล้ว" : historyStatus(selected.status)}
              date={selected.date}
...
function localDateKey(value: string) { const date = new Date(value); if (Number.isNaN(date.getTime())) return ""; const year = date.getFullYear(); const month = String(date.getMonth() + 1).padStart(2, "0"); const day = String(date.getDate()).padStart(2, "0"); return `${year}-${month}-${day}`; }
function historyStatus(status: string) { return ({ borrowed: "กำลังยืม", returned: "คืนแล้ว", overdue: "เกินกำหนด", reported: "แจ้งเสีย", inspecting: "กำลังตรวจสอบ", repairing: "กำลังดำเนินการ", completed: "ซ่อมเสร็จแล้ว", rejected: "ถูกปฏิเสธ", disposed: "จำหน่ายแล้ว" } as Record<string, string>)[status.toLowerCase()] || status; }
function maintenanceStatusClass(status: string) { return ({ reported: "bg-red-50 text-red-700", inspecting: "bg-amber-50 text-amber-700", repairing: "bg-blue-50 text-blue-700", completed: "bg-emerald-50 text-emerald-700", rejected: "bg-red-50 text-red-700", disposed: "bg-slate-100 text-slate-700" } as Record<string, string>)[status.toLowerCase()] || "bg-orange-50 text-orange-700"; }
