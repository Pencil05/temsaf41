...
function historyStatus(status: string) { return ({ borrowed: "กำลังยืม", returned: "คืนแล้ว", overdue: "เกินกำหนด", reported: "แจ้งเสีย", inspecting: "กำลังตรวจสอบ", repairing: "กำลังดำเนินการ", completed: "ซ่อมเสร็จแล้ว", disposed: "จำหน่ายแล้ว" } as Record<string, string>)[status.toLowerCase()] || status; }
function maintenanceStatusClass(status: string) { return ({ reported: "bg-red-50 text-red-700", inspecting: "bg-amber-50 text-amber-700", repairing: "bg-blue-50 text-blue-700", completed: "bg-emerald-50 text-emerald-700", disposed: "bg-slate-100 text-slate-700" } as Record<string, string>)[status.toLowerCase()] || "bg-orange-50 text-orange-700"; }
