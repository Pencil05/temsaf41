...
function InventoryBatchAddModal({ data, initialCompanyId, onClose, onConfirm }: { data: AdminData; initialCompanyId: string; onClose: () => void; onConfirm: (items: Array<{ companyId: string; equipmentId: string; total: number; plateNumber: string }>) => void }) {
  ...
  const updateRow = (key: string, patch: Partial<InventoryBatchAddRow>) => setRows((current) => current.map((row) => row.key === key ? { ...row, ...patch } : row));
  const submit = () => {
  ...
  return <div className="popup-backdrop fixed inset-0 z-[125] flex items-end justify-center bg-slate-950/55 sm:items-center sm:p-6" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div className="popup-panel flex max-h-[94dvh] w-full max-w-5xl flex-col overflow-hidden rounded-t-[30px] bg-white sm:rounded-[30px]"><div className="flex items-start justify-between border-b border-slate-200 p-5"><div><p className="text-xs font-bold uppercase tracking-wider text-blue-600">Batch Inventory</p><h2 className="mt-1 text-xl font-bold">เพิ่มยุทโธปกรณ์หลายรายการเข้าหลายกองร้อย</h2><p className="mt-1 text-sm text-slate-500">เพิ่มรายการได้หลายแถว ระบบจะรวมยอดชนิดที่ซ้ำกันให้อัตโนมัติ</p></div><button type="button" onClick={onClose} className="grid size-10 place-items-center rounded-xl bg-slate-100"><X className="size-5" /></button></div><div className="flex-1 space-y-3 overflow-y-auto bg-slate-50 p-4 sm:p-5">{rows.map((row, index) => { const equipment = data.equipments.find((item) => item.id === row.equipmentId); return <section key={row.key} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="mb-3 flex items-center justify-between"><div className="flex items-center gap-2"><span className="grid size-8 place-items-center rounded-lg bg-blue-50 text-sm font-black text-blue-700">{index + 1}</span><span className="text-sm font-bold">รายการเข้าคลัง</span></div><button type="button" disabled={rows.length === 1} onClick={() => setRows((current) => current.filter((item) => item.key !== row.key))} className="grid size-9 place-items-center rounded-lg text-red-500 transition hover:bg-red-50 disabled:opacity-30" aria-label="ลบแถว"><Trash2 className="size-4" /></button></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-[1fr_1.3fr_150px_1fr]"><SelectControl label="กองร้อย" value={row.companyId} onChange={(companyId) => updateRow(row.key, { companyId })} options={companyOptions} /><SelectControl label="ชนิดยุทโธปกรณ์" value={row.equipmentId} onChange={(equipmentId) => { const next = data.equipments.find((item) => item.id === equipmentId); updateRow(row.key, { equipmentId, total: next?.requirePlate ? "1" : row.total || "1", plateNumber: next?.requirePlate ? row.plateNumber : "" }); }} options={equipmentOptions} /><label><span className="mb-1 block text-sm font-semibold">จำนวน</span><input type="number" min={1} max={equipment?.requirePlate ? 1 : undefined} value={equipment?.requirePlate ? "1" : row.total} disabled={equipment?.requirePlate} onChange={(event) => updateRow(row.key, { total: event.target.value })} onBlur={() => { if (!row.total) updateRow(row.key, { total: "1" }); }} className="h-11 w-full rounded-xl border border-slate-200 px-3 disabled:bg-slate-100" /></label><label><span className="mb-1 block text-sm font-semibold">Serial/ทะเบียน</span><input value={row.plateNumber} disabled={!equipment?.requirePlate} required={Boolean(equipment?.requirePlate)} onChange={(event) => updateRow(row.key, { plateNumber: event.target.value })} placeholder={equipment?.requirePlate ? "กรอกหมายเลข" : "ไม่ต้องระบุ"} className="h-11 w-full rounded-xl border border-slate-200 px-3 disabled:bg-slate-100 disabled:text-slate-400" /></label></div>{equipment && <div className="mt-3 flex items-center gap-3 rounded-xl bg-blue-50/70 p-2.5"><Image src={equipment.picture} alt={equipment.name} width={42} height={42} unoptimized className="size-11 rounded-lg bg-white object-contain p-1" /><div className="min-w-0"><p className="truncate text-sm font-bold text-slate-800">{equipment.name}</p><p className="text-xs text-slate-500">{equipment.category} · {equipment.requirePlate ? "แยกรายคัน/Serial" : "นับตามจำนวน"}</p></div></div>}</section>; })}<button type="button" onClick={() => setRows((current) => [...current, createRow()])} className="flex h-12 w-full items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-blue-300 bg-blue-50/50 font-bold text-blue-700 transition hover:bg-blue-50"><Plus className="size-5" />เพิ่มแถวรายการ</button>{error && <p className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm font-semibold text-red-700">{error}</p>}</div><div className="grid grid-cols-2 gap-3 border-t border-slate-200 bg-white p-4 sm:p-5"><button type="button" onClick={onClose} className="h-12 rounded-xl bg-slate-100 font-bold text-slate-600">ยกเลิก</button><button type="button" onClick={submit} className="h-12 rounded-xl bg-blue-600 font-bold text-white">ยืนยันเพิ่ม {rows.length} รายการ</button></div></div></div>;
}
