    ...
    <div className="admin-shell min-h-screen bg-slate-50 text-slate-900">
      ...
      <div className="lg:pl-72">
        ...
        <main className="mx-auto max-w-7xl p-4 sm:p-6">
          ...
          {tab === "overview" && <Overview data={initialData} onEquipment={() => setEquipmentSummary(true)} />}
...
function Overview({ data, onEquipment }: { data: AdminData; onEquipment: () => void }) {
  const metrics = [
  ...
  return <section className="mt-6 grid grid-cols-2 gap-3 lg:grid-cols-4">{metrics.map(({ label, value, Icon }, index) => <button type="button" key={label} onClick={index === 0 ? onEquipment : undefined} className={`rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-md ${index === 0 ? "transition hover:-translate-y-0.5 hover:border-blue-400" : "cursor-default"}`}><Icon className="size-5 text-blue-600" /><p className="mt-5 text-sm text-slate-500">{label}</p><p className="text-3xl font-bold">{value.toLocaleString("th-TH")}</p>{index === 0 && <span className="mt-2 block text-xs font-semibold text-blue-600">คลิกดูรายละเอียดทั้งหมด</span>}</button>)}</section>;
}
...
function auditAction(action: string) { const key = action.toLowerCase().replace(/-/g, "_").replace(/^admin_/, ""); return ({ borrow: "เบิกยุทโธปกรณ์", return: "คืนยุทโธปกรณ์", defect: "แจ้งยุทโธปกรณ์ชำรุด", maintenance_status: "อัปเดตสถานะซ่อม", dispose_maintenance: "จำหน่ายยุทโธปกรณ์", transfer_inventory: "เคลื่อนย้ายยุทโธปกรณ์", save_user: "จัดการผู้ใช้งาน", save_equipment: "จัดการบัญชียุทโธปกรณ์", save_inventory: "ปรับปรุงคลัง", add_inventory: "เพิ่มรายการเข้าคลัง", save_company: "จัดการกองร้อย" } as Record<string, string>)[key] || action || "ไม่ระบุการดำเนินการ"; }
...
function EditModal({ modal, data, onClose, onSave }: { modal: NonNullable<Modal>; data: AdminData; onClose: () => void; onSave: (payload: Record<string, unknown>) => void }) {
  ...
  return <div className="popup-backdrop fixed inset-0 z-[100] flex items-end justify-center bg-slate-950/50 sm:items-center sm:p-6" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><form onSubmit={submit} className="popup-panel max-h-[92dvh] w-full max-w-xl overflow-y-auto rounded-t-[30px] bg-white p-5 sm:rounded-[30px]"><div className="flex justify-between"><div><p className="text-xs font-bold uppercase tracking-wider text-blue-600">Admin Action</p><h2 className="text-xl font-bold">{modal.type === "transfer" ? "เคลื่อนย้ายยุทโธปกรณ์ระหว่างกองร้อย" : modal.type === "inventory-add" ? "เพิ่มรายการเข้าคลัง" : modal.type === "inventory" ? "จัดการอาวุธในคลัง" : "จัดการข้อมูล"}</h2></div><button type="button" onClick={onClose}><X /></button></div><input type="hidden" name="action" value={action} />{!["inventory", "inventory-add", "transfer"].includes(modal.type) && <input type="hidden" name="id" value={String(item.id || "")} />}<div className="mt-5 grid gap-4 sm:grid-cols-2">
    ...
    {modal.type === "inventory" && <InventoryEditor data={data} initialCompanyId={String(item.companyId || "")} />}
...
function InventoryEditor({ data, initialCompanyId }: { data: AdminData; initialCompanyId: string }) {
  const initialStock = data.inventories.filter((item) => !initialCompanyId || item.companyId === initialCompanyId);
  const [inventoryId, setInventoryId] = useState(initialStock[0]?.id || data.inventories[0]?.id || "");
