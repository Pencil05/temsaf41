...
export function AdminConsole({ initialData, adminName }: { initialData: AdminData; adminName: string }) {
  ...
  const [inventorySerialOnly, setInventorySerialOnly] = useState(false);
  ...
  const filtered = useMemo(() => ({
    ...
    inventories: initialData.inventories.filter((item) => (!inventoryCompany || item.companyId === inventoryCompany) && (!inventorySerialOnly || item.requirePlate) && match(item.equipmentName, item.category, item.companyName, item.plateNumber)),
  ...
  }), [initialData, inventoryCompany, inventorySerialOnly, logCompany, logDate, maintenanceCompany, maintenanceDate, maintenanceView, match, transactionCompany, transactionDate]);
        ...
        <main className="mx-auto max-w-7xl p-4 sm:p-6">
          ...
          {tab === "inventory" && (
            <Section title={inventoryCompany ? `คลัง ${initialData.companies.find((item) => item.id === inventoryCompany)?.name}` : "คลังทุกกองร้อย"}>
              <Toolbar>
                ...
                <button type="button" onClick={() => { setInventorySerialOnly((current) => !current); setSelectedInventoryIds(new Set()); }} className={`flex h-11 items-center gap-2 rounded-xl border px-4 text-sm font-bold transition ${inventorySerialOnly ? "border-violet-600 bg-violet-600 text-white" : "border-violet-200 bg-white text-violet-700 hover:bg-violet-50"}`}><Hash className="size-4" />{inventorySerialOnly ? "กำลังแสดงรายการ Serial" : "จัดการ Serial"}</button>
              ...
              {inventorySerialOnly && <div className="mb-3 rounded-2xl border border-violet-200 bg-violet-50 px-4 py-3 text-sm text-violet-800">กำลังแสดงเฉพาะยุทโธปกรณ์ที่ลงทะเบียน Serial/ทะเบียน คลิกแต่ละแถวเพื่อแก้หมายเลขได้โดยตรง</div>}
          ...
          {tab === "transactions" && (
            <Section title="ประวัติการเบิกและคืนทั้งหมด">
              ...
              <AdminTransactionTable key={`${transactionCompany}:${transactionDate}:${keyword}`} items={filtered.transactions} onSelect={setReceipt} onDelete={(ids) => setHistoryDelete({ ids, all: false, count: ids.length })} />
            </Section>
          )}
...
function AdminTransactionTable({ items, onSelect, onDelete }: { items: AdminTransaction[]; onSelect: (item: AdminTransaction) => void; onDelete: (ids: string[]) => void }) {
...
function BatchInventoryModal({ items, onClose, onConfirm }: { items: AdminData["inventories"]; onClose: () => void; onConfirm: (items: Array<{ id: string; total: number }>) => void }) {
  ...
  const changed = items.filter((item) => totals[item.id] !== item.total);
  return <div className="popup-backdrop fixed inset-0 z-[125] flex items-end justify-center bg-slate-950/55 sm:items-center sm:p-6" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div className="popup-panel flex max-h-[92dvh] w-full max-w-4xl flex-col overflow-hidden rounded-t-[30px] bg-white sm:rounded-[30px]"><div className="flex items-start justify-between border-b border-slate-200 p-5"><div><p className="text-xs font-bold uppercase tracking-wider text-emerald-600">Batch Inventory</p><h2 className="mt-1 text-xl font-bold">ปรับจำนวนยุทโธปกรณ์หลายรายการ</h2><p className="mt-1 text-sm text-slate-500">เลือกไว้ {items.length.toLocaleString("th-TH")} รายการ · มีการเปลี่ยนแปลง {changed.length.toLocaleString("th-TH")} รายการ</p></div><button type="button" onClick={onClose} className="grid size-10 place-items-center rounded-xl bg-slate-100"><X className="size-5" /></button></div><div className="flex-1 space-y-3 overflow-y-auto bg-slate-50 p-4 sm:p-5">{items.map((item) => { const minimum = Math.max(1, item.borrowed + item.broken); const total = totals[item.id] ?? item.total; return <div key={item.id} className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:grid-cols-[minmax(0,1fr)_220px]"><div className="flex min-w-0 items-center gap-3"><span className="grid size-14 shrink-0 place-items-center overflow-hidden rounded-xl bg-slate-50 p-1"><Image src={item.picture} alt={item.equipmentName} width={56} height={56} unoptimized className="size-full object-contain" /></span><span className="min-w-0"><span className="block truncate font-bold">{item.equipmentName}</span><span className="mt-1 block text-xs text-slate-500">{item.companyName}{item.plateNumber ? ` · Serial ${item.plateNumber}` : ""}</span><span className="mt-1 block text-xs text-slate-500">เดิม {item.total.toLocaleString("th-TH")} · ยืม {item.borrowed.toLocaleString("th-TH")} · ชำรุด {item.broken.toLocaleString("th-TH")}</span></span></div><div><span className="mb-1 block text-xs font-semibold text-slate-500">จำนวนรวมใหม่</span><div className="flex items-center gap-2"><button type="button" disabled={item.requirePlate || total <= minimum} onClick={() => update(item, total - 1)} className="grid size-10 shrink-0 place-items-center rounded-xl border border-slate-200 bg-white disabled:opacity-30"><Minus className="size-4" /></button><input type="number" value={total} min={minimum} disabled={item.requirePlate} onChange={(event) => update(item, Number(event.target.value))} className="h-10 min-w-0 flex-1 rounded-xl border border-slate-200 px-3 text-center font-bold disabled:bg-slate-100" /><button type="button" disabled={item.requirePlate} onClick={() => update(item, total + 1)} className="grid size-10 shrink-0 place-items-center rounded-xl border border-slate-200 bg-white disabled:opacity-30"><Plus className="size-4" /></button></div>{item.requirePlate && <span className="mt-1 block text-[11px] text-violet-600">รายการ Serial ถูกล็อกจำนวนไว้ที่ 1</span>}{!item.requirePlate && minimum > 1 && <span className="mt-1 block text-[11px] text-amber-600">ลดได้ต่ำสุด {minimum} เพราะมียอดยืมหรือชำรุด</span>}</div></div>; })}</div><div className="grid grid-cols-2 gap-3 border-t border-slate-200 bg-white p-4 sm:p-5"><button type="button" onClick={onClose} className="h-12 rounded-xl bg-slate-100 font-bold text-slate-600">ยกเลิก</button><button type="button" disabled={!changed.length} onClick={() => onConfirm(items.map((item) => ({ id: item.id, total: totals[item.id] ?? item.total })))} className="h-12 rounded-xl bg-emerald-600 font-bold text-white disabled:opacity-40">ยืนยันปรับ {changed.length} รายการ</button></div></div></div>;
}
