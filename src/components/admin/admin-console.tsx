...
function Overview({ data, onEquipment, onUsers, onBorrowed, onMaintenance }: { data: AdminData; onEquipment: () => void; onUsers: () => void; onBorrowed: () => void; onMaintenance: () => void }) {
  const metrics = [
    ...
    { label: "กำลังยืม", value: data.transactions.filter((item) => item.movementType === "borrow" && ["borrowed", "overdue"].includes(item.status.toLowerCase())).length, Icon: Activity, onClick: onBorrowed, hint: "ดูผู้ยืมและรายการทั้งหมด" },
...
function BorrowedSummaryModal({ data, onClose, onSelect, onReturn }: { data: AdminData; onClose: () => void; onSelect: (item: AdminTransaction) => void; onReturn: (item: AdminTransaction) => void }) {
  ...
  const active = data.transactions
    .filter((item) => item.movementType === "borrow" && ["borrowed", "overdue"].includes(item.status.toLowerCase()))
...
function auditAction(action: string) { const key = action.toLowerCase().replace(/-/g, "_").replace(/^admin_/, ""); return ({ borrow: "เบิกยุทโธปกรณ์", return: "คืนยุทโธปกรณ์", return_transaction: "คืนยุทโธปกรณ์", defect: "แจ้งยุทโธปกรณ์ชำรุด", maintenance_status: "อัปเดตสถานะซ่อม", dispose_maintenance: "จำหน่ายยุทโธปกรณ์", transfer_inventory: "เคลื่อนย้ายยุทโธปกรณ์", save_user: "จัดการผู้ใช้งาน", delete_user: "ลบผู้ใช้งาน", delete_company: "ลบกองร้อย", save_equipment: "จัดการบัญชียุทโธปกรณ์", save_inventory: "ปรับปรุงคลัง", add_inventory: "เพิ่มรายการเข้าคลัง", delete_inventory: "ลบรายการออกจากคลัง", save_company: "จัดการกองร้อย" } as Record<string, string>)[key] || action || "ไม่ระบุการดำเนินการ"; }
