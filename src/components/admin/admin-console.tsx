...
export function AdminConsole({ initialData, initialOperations, adminName }: { initialData: AdminData; initialOperations: AdminOperationsData; adminName: string }) {
  ...
  const [maintenanceHistoryDelete, setMaintenanceHistoryDelete] = useState<{ ids: string[]; all: boolean; count: number } | null>(null);
  ...
  usePopupDismiss(Boolean(maintenanceHistoryDelete), () => setMaintenanceHistoryDelete(null));
  ...
  return (
    <div className="admin-shell min-h-screen overflow-x-hidden bg-slate-50 text-slate-900">
          ...
          {tab === "maintenance" && (
            <Section title={maintenanceView === "active" ? "รายการซ่อมบำรุงปัจจุบัน" : "ประวัติการส่งซ่อม"}>
              <Toolbar>
                ...
                {maintenanceView === "history" && <button type="button" disabled={!filtered.maintenance.length} onClick={() => setMaintenanceHistoryDelete({ ids: [], all: true, count: filtered.maintenance.length })} className="flex h-11 items-center gap-2 rounded-xl border border-red-200 bg-red-50 px-4 text-sm font-bold text-red-700 disabled:opacity-40"><Trash2 className="size-4" />ล้างประวัติทั้งหมด</button>}
              </Toolbar>
              <MaintenanceTable items={filtered.maintenance} active={maintenanceView === "active"} onSelect={setMaintenanceReceipt} onStatus={(item, status) => mutate({ action: "maintenance-status", id: item.id, status })} onDispose={setDisposeItem} onDelete={(item) => setMaintenanceHistoryDelete({ ids: [item.id], all: false, count: 1 })} emptyText={maintenanceView === "active" ? "ไม่มีรายการที่กำลังซ่อม" : "ยังไม่มีประวัติซ่อมเสร็จ"} />
            </Section>
          )}
      ...
      {maintenanceHistoryDelete && <ConfirmMaintenanceHistoryDeleteModal request={maintenanceHistoryDelete} onClose={() => setMaintenanceHistoryDelete(null)} onConfirm={() => { const request = maintenanceHistoryDelete; setMaintenanceHistoryDelete(null); mutate({ action: "delete-maintenance-history", ids: request.ids, all: request.all }); }} />}
...
function Overview({ data, onEquipment, onUsers, onBorrowed, onMaintenance }: { data: AdminData; onEquipment: () => void; onUsers: () => void; onBorrowed: () => void; onMaintenance: () => void }) {
  const metrics = [
    ...
    { label: "กำลังซ่อม", value: data.maintenance.filter((item) => !["completed", "disposed", "rejected"].includes(item.status.toLowerCase())).length, Icon: Wrench, onClick: onMaintenance, hint: "เปิดหน้าซ่อมบำรุง" },
  ];
...
function ConfirmMaintenanceHistoryDeleteModal({ request, onClose, onConfirm }: { request: { ids: string[]; all: boolean; count: number }; onClose: () => void; onConfirm: () => void }) {
  return <div className="popup-backdrop fixed inset-0 z-[150] flex items-center justify-center bg-slate-950/60 p-5" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div className="popup-panel w-full max-w-md rounded-[24px] bg-white p-5"><span className="grid size-12 place-items-center rounded-xl bg-red-50 text-red-600"><Trash2 className="size-6" /></span><h2 className="mt-4 text-xl font-bold">{request.all ? "ล้างประวัติซ่อมเสร็จทั้งหมด" : "ลบประวัติซ่อมรายการนี้"}</h2><p className="mt-2 text-sm leading-6 text-slate-600">ระบบจะลบประวัติซ่อมที่ปิดงานแล้วจำนวน {request.count.toLocaleString("th-TH")} รายการ รายการที่กำลังตรวจสอบหรือกำลังซ่อมจะไม่ถูกลบ</p><div className="mt-5 grid grid-cols-2 gap-3"><button type="button" onClick={onClose} className="h-11 rounded-xl bg-slate-100 font-semibold">ยกเลิก</button><button type="button" onClick={onConfirm} className="h-11 rounded-xl bg-red-600 font-semibold text-white">ยืนยันลบประวัติ</button></div></div></div>;
}
...
function maintenanceStatus(status: string) { return ({ reported: "แจ้งเสีย", inspecting: "กำลังตรวจสอบ", repairing: "กำลังดำเนินการ", completed: "ซ่อมเสร็จแล้ว", rejected: "ถูกปฏิเสธ", disposed: "จำหน่ายแล้ว" } as Record<string, string>)[status.toLowerCase()] || status; }
function auditAction(action: string) { const key = action.toLowerCase().replace(/-/g, "_").replace(/^admin_/, ""); return ({ borrow: "เบิกยุทโธปกรณ์", return: "คืนยุทโธปกรณ์", return_transaction: "คืนยุทโธปกรณ์", delete_transaction_history: "ล้างประวัติเบิก / คืน", delete_maintenance_history: "ล้างประวัติซ่อมเสร็จ", defect: "แจ้งยุทโธปกรณ์ชำรุด", report_defect: "แจ้งยุทโธปกรณ์ชำรุด", maintenance_status: "อัปเดตสถานะซ่อม", dispose_maintenance: "จำหน่ายยุทโธปกรณ์", transfer_inventory: "เคลื่อนย้ายยุทโธปกรณ์", save_user: "จัดการผู้ใช้งาน", delete_user: "ลบผู้ใช้งาน", delete_company: "ลบกองร้อย", save_equipment: "จัดการบัญชียุทโธปกรณ์", save_category: "จัดการหมวดหมู่ยุทโธปกรณ์", delete_equipment: "ลบชนิดยุทโธปกรณ์", delete_equipment_category: "ลบหมวดหมู่ยุทโธปกรณ์", save_inventory: "ปรับปรุงคลัง", batch_adjust_inventory: "ปรับจำนวนยุทโธปกรณ์หลายรายการ", add_inventory: "เพิ่มรายการเข้าคลัง", batch_add_inventory: "เพิ่มยุทโธปกรณ์หลายรายการเข้าคลัง", delete_inventory: "ลบรายการออกจากคลัง", save_company: "จัดการกองร้อย" } as Record<string, string>)[key] || action || "ไม่ระบุการดำเนินการ"; }
