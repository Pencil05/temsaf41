...
type Modal = { type: "company" | "user" | "equipment" | "inventory" | "inventory-add" | "transfer"; item?: Record<string, unknown> } | null;
          ...
          {tab === "inventory" && (
            <Section title={inventoryCompany ? `คลัง ${initialData.companies.find((item) => item.id === inventoryCompany)?.name}` : "คลังทุกกองร้อย"}>
              <Toolbar>
                ...
                <button type="button" onClick={() => setModal({ type: "inventory-add", item: { companyId: inventoryCompany } })} className="flex h-11 items-center gap-2 rounded-xl bg-blue-600 px-4 text-sm font-bold text-white"><PackagePlus className="size-4" />เพิ่มรายการเข้าคลัง</button>
              ...
              <InventoryTable items={filtered.inventories} groupByCompany={!inventoryCompany} onEdit={(item) => setModal({ type: "inventory", item: item as unknown as Record<string, unknown> })} onDelete={setDeleteInventoryItem} />
            </Section>
          )}

          {tab === "transactions" && (
            <Section title="ประวัติการเบิกและคืนทั้งหมด">
              <Toolbar><CompanyFilter value={transactionCompany} onChange={setTransactionCompany} companies={initialData.companies} /><DateFilter value={transactionDate} onChange={setTransactionDate} /></Toolbar>
...
function Overview({ data, onEquipment, onUsers, onBorrowed, onMaintenance }: { data: AdminData; onEquipment: () => void; onUsers: () => void; onBorrowed: () => void; onMaintenance: () => void }) {
  const metrics = [
    ...
    { label: "กำลังยืม", value: data.transactions.filter((item) => ["borrowed", "overdue"].includes(item.status.toLowerCase())).length, Icon: Activity, onClick: onBorrowed, hint: "ดูผู้ยืมและรายการทั้งหมด" },
...
function InventoryTable({ items, onEdit, onDelete, groupByCompany = false }: { items: AdminData["inventories"]; onEdit?: (item: AdminData["inventories"][number]) => void; onDelete?: (item: AdminData["inventories"][number]) => void; groupByCompany?: boolean }) {
  ...
  return <div className="mt-3 overflow-x-auto rounded-2xl border border-slate-200 bg-white"><table className="w-full min-w-[1040px] text-sm"><thead className="bg-slate-50"><tr>{["เลขที่คลัง", "ยุทโธปกรณ์", "หมวดหมู่", "กองร้อย", "ทะเบียน", "รวม", "พร้อม", "ให้ยืม", "เสีย", "จัดการ"].map((label) => <th key={label} className="p-3 text-left">{label}</th>)}</tr></thead><tbody>{ordered.map((item, index) => { const newCompany = groupByCompany && item.companyName !== ordered[index - 1]?.companyName; const hidden = collapsed.has(item.companyName); return <Fragment key={item.id}>{newCompany && <tr><td colSpan={10} className="border-t border-blue-200 bg-blue-50 p-0"><button type="button" onClick={() => setCollapsed((current) => { const next = new Set(current); if (next.has(item.companyName)) next.delete(item.companyName); else next.add(item.companyName); return next; })} className="flex w-full items-center gap-2 px-4 py-3 font-bold text-blue-800"><ChevronDown className={`size-4 transition-transform ${hidden ? "-rotate-90" : ""}`} /><Building2 className="size-4" />{item.companyName}</button></td></tr>}{(!groupByCompany || !hidden) && <tr onClick={() => onEdit?.(item)} className={`border-t border-slate-100 ${onEdit ? "cursor-pointer hover:bg-blue-50" : ""}`}><DocumentCell id={item.id} /><td className="p-3 font-semibold">{item.equipmentName}</td><td>{item.category}</td><td>{item.companyName}</td><td>{item.plateNumber || "-"}</td><td>{item.total}</td><td className="text-emerald-600">{item.available}</td><td className="text-amber-600">{item.borrowed}</td><td className="text-red-600">{item.broken}</td><td className="p-2">{onDelete && <button type="button" onClick={(event) => { event.stopPropagation(); onDelete(item); }} className="grid size-9 place-items-center rounded-lg border border-red-200 bg-red-50 text-red-600 transition hover:bg-red-100" aria-label={`ลบ ${item.equipmentName}`}><Trash2 className="size-4" /></button>}</td></tr>}</Fragment>; })}</tbody></table>{!items.length && <Empty text="ไม่พบรายการในคลัง" />}</div>;
}
...
function AdminTransactionTable({ items, onSelect }: { items: AdminTransaction[]; onSelect: (item: AdminTransaction) => void }) {
  ...
  const [collapsed, setCollapsed] = useState<Set<string>>(new Set());
  return <DatedTableShell groups={groups} collapsed={collapsed} setCollapsed={setCollapsed} emptyText="ไม่พบประวัติการเบิกหรือคืนตามตัวกรอง" render={(records) => <div className="overflow-x-auto"><table className="w-full min-w-[1120px] text-sm"><thead className="border-y border-slate-200 text-left text-xs text-slate-500"><tr>{["เลขที่เอกสาร", "สถานะ", "ยุทโธปกรณ์", "จำนวน", "ต้นทาง", "ปลายทาง", "ผู้ดำเนินการ", "เวลา", ""].map((label) => <th key={label} className="px-4 py-2 font-medium">{label}</th>)}</tr></thead><tbody>{records.map((item) => { const open = () => onSelect(item); const returned = item.status.toLowerCase() === "returned"; return <tr key={item.id} tabIndex={0} onClick={open} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); open(); } }} className="cursor-pointer border-t border-slate-100 hover:bg-blue-50 focus-visible:bg-blue-50 focus-visible:outline-none"><DocumentCell id={item.id} /><td className="px-4 py-2.5"><span className={`rounded-full px-2 py-1 text-xs font-semibold ${returned ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{transactionStatus(item.status)}</span></td><td className="max-w-[220px] truncate px-4 py-2.5 font-semibold">{item.equipmentName}</td><td className="px-4 py-2.5">{item.quantity.toLocaleString("th-TH")}</td><td className="max-w-[180px] truncate px-4 py-2.5">{returned ? item.borrower : item.owner}</td><td className="max-w-[180px] truncate px-4 py-2.5">{returned ? item.owner : item.borrower}</td><td className="max-w-[180px] truncate px-4 py-2.5 text-slate-600">{item.operator}</td><td className="whitespace-nowrap px-4 py-2.5 text-slate-500">{formatTime(item.returnDate || item.date)}</td><td className="px-4 py-2 text-right"><button type="button" onClick={(event) => { event.stopPropagation(); open(); }} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-slate-200 px-3 text-xs font-semibold hover:border-blue-400 hover:text-blue-600"><Eye className="size-3.5" />ดูใบเสร็จ</button></td></tr>; })}</tbody></table></div>} />;
}
...
function BorrowedSummaryModal({ data, onClose, onSelect, onReturn }: { data: AdminData; onClose: () => void; onSelect: (item: AdminTransaction) => void; onReturn: (item: AdminTransaction) => void }) {
  ...
  const active = data.transactions
    .filter((item) => ["borrowed", "overdue"].includes(item.status.toLowerCase()))
...
function auditAction(action: string) { const key = action.toLowerCase().replace(/-/g, "_").replace(/^admin_/, ""); return ({ borrow: "เบิกยุทโธปกรณ์", return: "คืนยุทโธปกรณ์", return_transaction: "คืนยุทโธปกรณ์", defect: "แจ้งยุทโธปกรณ์ชำรุด", maintenance_status: "อัปเดตสถานะซ่อม", dispose_maintenance: "จำหน่ายยุทโธปกรณ์", transfer_inventory: "เคลื่อนย้ายยุทโธปกรณ์", save_user: "จัดการผู้ใช้งาน", delete_user: "ลบผู้ใช้งาน", save_equipment: "จัดการบัญชียุทโธปกรณ์", save_inventory: "ปรับปรุงคลัง", add_inventory: "เพิ่มรายการเข้าคลัง", delete_inventory: "ลบรายการออกจากคลัง", save_company: "จัดการกองร้อย" } as Record<string, string>)[key] || action || "ไม่ระบุการดำเนินการ"; }
...
function EquipmentEditor({ item, categories }: { item: Record<string, unknown>; categories: string[] }) {
  ...
  async function upload(event: React.ChangeEvent<HTMLInputElement>) { const file = event.target.files?.[0]; if (!file) return; if (!file.type.startsWith("image/") || file.size > 5 * 1024 * 1024) { event.target.value = ""; return; } setPreparing(true); try { setPicture(await compressImageForSheet(file)); } finally { setPreparing(false); } }
...
function InventoryEditor({ data, initialCompanyId, initialInventoryId }: { data: AdminData; initialCompanyId: string; initialInventoryId: string }) {
  ...
  const available = Math.max(0, total - (selected?.borrowed || 0) - (selected?.broken || 0));
  return <><div className="sm:col-span-2"><SelectControl label="กองร้อยและยุทโธปกรณ์ที่ต้องการจัดการ" value={inventoryId} onChange={(nextId) => { setInventoryId(nextId); setTotal(data.inventories.find((item) => item.id === nextId)?.total || 1); }} options={data.inventories.map((item) => [item.id, `${item.companyName} · ${item.equipmentName}${item.plateNumber ? ` · ${item.plateNumber}` : ""}`])} /></div><input type="hidden" name="id" value={selected?.id || ""} /><input type="hidden" name="companyId" value={selected?.companyId || ""} /><input type="hidden" name="equipmentId" value={selected?.equipmentId || ""} /><input type="hidden" name="plateNumber" value={selected?.plateNumber || ""} /><InputControl name="total" label="จำนวนรวม" value={total} onChange={setTotal} min={(selected?.borrowed || 0) + (selected?.broken || 0)} /><ReadOnlyNumber label="พร้อมใช้ (คำนวณอัตโนมัติ)" value={available} tone="text-emerald-700" /><ReadOnlyNumber label="ถูกยืม (แก้ไขไม่ได้)" value={selected?.borrowed || 0} tone="text-amber-700" /><ReadOnlyNumber label="ชำรุด" value={selected?.broken || 0} tone="text-red-700" /></>;
}
...
function TransferEditor({ data }: { data: AdminData }) {
  ...
  return <><SelectControl name="sourceInventoryId" label="รายการต้นทาง" value={sourceId} onChange={(nextId) => { setSourceId(nextId); setQuantity(1); }} options={available.map((item) => [item.id, `${item.companyName} · ${item.equipmentName}${item.plateNumber ? ` · ${item.plateNumber}` : ""} · มี ${item.available.toLocaleString("th-TH")}`])} /><Select name="destinationCompanyId" label="กองร้อยปลายทาง" options={data.companies.filter((company) => company.id !== source?.companyId).map((company) => [company.id, company.name])} /><InputControl name="quantity" label={`จำนวนที่เคลื่อนย้าย (มี ${source?.available || 0})`} value={cappedQuantity} onChange={setQuantity} min={1} max={source?.available || 1} /><div className="rounded-xl bg-amber-50 p-3 text-xs leading-5 text-amber-800">เคลื่อนย้ายได้สูงสุด {source?.available.toLocaleString("th-TH") || 0} รายการ ระบบจะปรับยอดต้นทางและปลายทางพร้อมบันทึก Audit Log</div></>;
}

function SelectControl({ label, value, onChange, options, name }: { label: string; value: string; onChange: (value: string) => void; options: string[][]; name?: string }) { return <label className="min-w-0"><span className="mb-1 block text-sm font-semibold">{label}</span><select name={name} value={value} onChange={(event) => onChange(event.target.value)} required className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3"><option value="">เลือกรายการ</option>{options.map(([optionValue, optionLabel]) => <option key={optionValue} value={optionValue}>{optionLabel}</option>)}</select></label>; }
