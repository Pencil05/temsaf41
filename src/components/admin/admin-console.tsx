        ...
        <main className="mx-auto max-w-7xl p-4 sm:p-6">
          ...
          {tab === "maintenance" && (
            <Section title={maintenanceView === "active" ? "รายการซ่อมบำรุงปัจจุบัน" : "ประวัติการส่งซ่อม"}>
              ...
              <MaintenanceTable items={filtered.maintenance} active={maintenanceView === "active"} onSelect={setMaintenanceReceipt} onStatus={(item, status) => mutate({ action: "maintenance-status", id: item.id, status })} emptyText={maintenanceView === "active" ? "ไม่มีรายการที่กำลังซ่อม" : "ยังไม่มีประวัติซ่อมเสร็จ"} />
            </Section>
          )}
...
function AdminTransactionTable({ items, onSelect }: { items: AdminTransaction[]; onSelect: (item: AdminTransaction) => void }) {
  ...
  return <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">{groups.map(([key, records]) => {
    ...
    return <div key={key} className="border-b border-slate-200 last:border-b-0"><button type="button" onClick={() => setCollapsed((current) => { const next = new Set(current); if (next.has(key)) next.delete(key); else next.add(key); return next; })} className="flex w-full items-center gap-3 bg-slate-50 px-4 py-3 text-left hover:bg-slate-100"><ChevronDown className={`size-4 transition-transform ${hidden ? "-rotate-90" : ""}`} /><span className="font-semibold">{key === "unknown" ? "ไม่ระบุวันที่" : new Intl.DateTimeFormat("th-TH", { dateStyle: "long" }).format(new Date(date))}</span><span className="ml-auto rounded-full bg-white px-2.5 py-1 text-xs text-slate-500">{records.length} รายการ</span></button>{!hidden && <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-sm"><thead className="border-y border-slate-200 text-left text-xs text-slate-500"><tr>{["สถานะ", "ยุทโธปกรณ์", "จำนวน", "ต้นทาง", "ปลายทาง", "ผู้ดำเนินการ", "เวลา", ""].map((label) => <th key={label} className="px-4 py-2 font-medium">{label}</th>)}</tr></thead><tbody>{records.map((item) => { const openReceipt = () => onSelect(item); return <tr key={item.id} tabIndex={0} onClick={openReceipt} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openReceipt(); } }} className="cursor-pointer border-t border-slate-100 hover:bg-blue-50 focus-visible:bg-blue-50 focus-visible:outline-none"><td className="px-4 py-2.5"><span className={`rounded-full px-2 py-1 text-xs font-semibold ${item.status.toLowerCase() === "returned" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{item.status}</span></td><td className="max-w-[220px] truncate px-4 py-2.5 font-semibold">{item.equipmentName}</td><td className="px-4 py-2.5">{item.quantity.toLocaleString("th-TH")}</td><td className="max-w-[180px] truncate px-4 py-2.5">{item.owner}</td><td className="max-w-[180px] truncate px-4 py-2.5">{item.borrower}</td><td className="max-w-[180px] truncate px-4 py-2.5 text-slate-600">{item.operator}</td><td className="whitespace-nowrap px-4 py-2.5 text-slate-500">{formatTime(item.returnDate || item.date)}</td><td className="px-4 py-2 text-right"><button type="button" onClick={(event) => { event.stopPropagation(); openReceipt(); }} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-slate-200 px-3 text-xs font-semibold hover:border-blue-400 hover:text-blue-600"><Eye className="size-3.5" />ดูใบเสร็จ</button></td></tr>; })}</tbody></table></div>}</div>;
  })}{!items.length && <Empty text="ไม่พบประวัติการเบิกหรือคืนตามตัวกรอง" />}</div>;
}

function MaintenanceTable({ items, active, onSelect, onStatus, emptyText }: { items: AdminMaintenance[]; active: boolean; onSelect: (item: AdminMaintenance) => void; onStatus: (item: AdminMaintenance, status: string) => void; emptyText: string }) {
  const groups = useMemo(() => groupRecordsByDate(items, (item) => item.completedAt || item.date), [items]);
  const [collapsed, setCollapsed] = useState<Set<string>>(new Set());
  return <DatedTableShell groups={groups} collapsed={collapsed} setCollapsed={setCollapsed} emptyText={emptyText} render={(records) => <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-sm"><thead className="border-y border-slate-200 text-left text-xs text-slate-500"><tr>{["สถานะ", "ยุทโธปกรณ์", "กองร้อย", "จำนวน", "ผู้แจ้ง", "หมายเหตุ", "เวลา", active ? "จัดการสถานะ" : "รายละเอียด"].map((label) => <th key={label} className="px-4 py-2 font-medium">{label}</th>)}</tr></thead><tbody>{records.map((item) => { const open = () => onSelect(item); return <tr key={item.id} tabIndex={0} onClick={open} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); open(); } }} className="cursor-pointer border-t border-slate-100 hover:bg-blue-50 focus-visible:bg-blue-50 focus-visible:outline-none"><td className="px-4 py-2.5"><span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-xs font-semibold ${item.status.toLowerCase() === "completed" ? "bg-emerald-50 text-emerald-700" : "bg-orange-50 text-orange-700"}`}>{item.status.toLowerCase() === "completed" && <CheckCircle2 className="size-3.5" />}{maintenanceStatus(item.status)}</span></td><td className="max-w-[220px] truncate px-4 py-2.5 font-semibold">{item.equipmentName}</td><td className="max-w-[180px] truncate px-4 py-2.5">{item.companyName}</td><td className="px-4 py-2.5">{item.quantity.toLocaleString("th-TH")}</td><td className="max-w-[180px] truncate px-4 py-2.5 text-slate-600">{item.operator}</td><td className="max-w-[240px] truncate px-4 py-2.5 text-slate-500">{item.note || "ไม่มีหมายเหตุ"}</td><td className="whitespace-nowrap px-4 py-2.5 text-slate-500">{formatTime(item.completedAt || item.date)}</td><td className="px-4 py-2" onClick={(event) => event.stopPropagation()}>{active ? <select value={item.status} onChange={(event) => onStatus(item, event.target.value)} className="h-9 rounded-lg border border-slate-200 bg-white px-2 text-xs font-semibold"><option value="Reported">รับแจ้ง</option><option value="Inspecting">กำลังตรวจสอบ</option><option value="Repairing">กำลังซ่อม</option><option value="Completed">ซ่อมเสร็จ</option></select> : <button type="button" onClick={open} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-slate-200 px-3 text-xs font-semibold hover:border-blue-400 hover:text-blue-600"><Eye className="size-3.5" />ดูใบเสร็จ</button>}</td></tr>; })}</tbody></table></div>} />;
}
...
function AuditLogTable({ items, onSelect }: { items: AdminAuditLog[]; onSelect: (item: AdminAuditLog) => void }) {
  ...
  const [collapsed, setCollapsed] = useState<Set<string>>(new Set());
  return <DatedTableShell groups={groups} collapsed={collapsed} setCollapsed={setCollapsed} emptyText="ไม่พบ Audit Log ตามตัวกรอง" render={(records) => <div className="overflow-x-auto"><table className="w-full min-w-[940px] text-sm"><thead className="border-y border-slate-200 text-left text-xs text-slate-500"><tr>{["การดำเนินการ", "ผู้ดำเนินการ", "รายการที่เกี่ยวข้อง", "กองร้อย", "รายละเอียดโดยย่อ", "เวลา"].map((label) => <th key={label} className="px-4 py-2 font-medium">{label}</th>)}</tr></thead><tbody>{records.map((item) => { const open = () => onSelect(item); return <tr key={item.id} tabIndex={0} onClick={open} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); open(); } }} className="cursor-pointer border-t border-slate-100 hover:bg-blue-50 focus-visible:bg-blue-50 focus-visible:outline-none"><td className="px-4 py-2.5"><span className="rounded-full bg-blue-50 px-2 py-1 text-xs font-semibold text-blue-700">{auditAction(item.action)}</span></td><td className="max-w-[190px] truncate px-4 py-2.5 font-semibold">{item.user}</td><td className="max-w-[220px] truncate px-4 py-2.5">{item.targetLabel}</td><td className="max-w-[220px] truncate px-4 py-2.5 text-slate-600">{item.companyNames.join(", ") || "ไม่ระบุกองร้อย"}</td><td className="max-w-[280px] truncate px-4 py-2.5 text-slate-500">{item.details || "ไม่มีรายละเอียดเพิ่มเติม"}</td><td className="whitespace-nowrap px-4 py-2.5 text-slate-500">{formatTime(item.timestamp)}</td></tr>; })}</tbody></table></div>} />;
}
...
function maintenanceStatus(status: string) { return ({ reported: "รับแจ้ง", inspecting: "กำลังตรวจสอบ", repairing: "กำลังซ่อม", completed: "ซ่อมเสร็จ" } as Record<string, string>)[status.toLowerCase()] || status; }
function auditAction(action: string) { return ({ borrow: "เบิกยุทโธปกรณ์", return: "คืนยุทโธปกรณ์", defect: "แจ้งยุทโธปกรณ์ชำรุด", maintenance_status: "อัปเดตสถานะซ่อม", transfer_inventory: "เคลื่อนย้ายยุทโธปกรณ์", save_user: "จัดการผู้ใช้งาน", save_equipment: "จัดการบัญชียุทโธปกรณ์", save_inventory: "ปรับปรุงคลัง", save_company: "จัดการกองร้อย" } as Record<string, string>)[action.toLowerCase().replace(/-/g, "_")] || action || "ไม่ระบุการดำเนินการ"; }
