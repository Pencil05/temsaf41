...
export function CategoryInventoryClient({ data, initialEquipment = "" }: { data: CategoryInventoryData; initialEquipment?: string }) {
  ...
  function submitBorrow(event: FormEvent<HTMLFormElement>) {
    ...
    if (!selfUse && activeItem.stockType === "borrowed") {
          ...
          {data.inventory.length ? data.inventory.map((item) => (
            ...
            >
              ...
              <div className="p-4">
                ...
                {item.stockType === "borrowed" && <p className="mt-3 rounded-xl bg-amber-50 px-3 py-2 text-xs font-bold text-amber-700">ของยืมมา · เบิกใช้งานได้เฉพาะภายในหน่วย</p>}
              </div>
            </button>
          )) : (
        ...
        <div className="popup-backdrop fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/50 p-0 backdrop-blur-sm sm:items-center sm:p-6" role="dialog" aria-modal="true" onMouseDown={(event) => { if (event.target === event.currentTarget) setActiveItem(null); }}>
          <form onSubmit={submitBorrow} className="popup-panel w-full max-w-md rounded-t-[30px] bg-white p-5 shadow-2xl sm:rounded-[30px] sm:p-6">
            ...
            <div className="mt-5 space-y-4">
              ...
              {!selfUse && companyId && activeItem.stockType === "borrowed" && <div className="rounded-2xl border border-red-200 bg-red-50 p-3 text-sm font-semibold leading-6 text-red-700">รายการนี้เป็นของยืมมา ระบบอนุญาตเฉพาะการเบิกใช้งานภายในหน่วยและการคืนเจ้าของเดิม</div>}
            ...
            <div className="mt-6 grid grid-cols-2 gap-3"><button type="button" onClick={() => setActiveItem(null)} className="h-12 rounded-full bg-slate-100 font-bold text-slate-600">ยกเลิก</button><button type="submit" disabled={isSubmitting || activeItem.available < 1 || (!selfUse && Boolean(companyId) && activeItem.stockType === "borrowed")} className="h-12 rounded-full bg-emerald-600 font-bold text-white shadow-lg shadow-emerald-100 disabled:opacity-60">{isSubmitting ? "กำลังบันทึก..." : "ยืนยัน"}</button></div>
          </form>
        </div>
      )}
